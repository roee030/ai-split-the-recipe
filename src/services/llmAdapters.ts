...
// Cast to plain map — Vite bundles the JSON at build time so corrections
// are always in sync with what the regression suite last wrote.
const AUTO_CORRECTIONS = _autoCorrections as Record<string, string>;

/**
 * Apply auto-learned corrections to every item name in the receipt.
 * These are generated by `npm run regression -- --apply-corrections` and
 * stored in src/data/autoLearnedCorrections.json.
 * They act as a global lookup table that runs BEFORE per-restaurant corrections.
 */
function applyAutoCorrections(receipt: ParsedReceipt): ParsedReceipt {
  if (!Object.keys(AUTO_CORRECTIONS).length) return receipt;
  return {
    ...receipt,
    items: receipt.items.map(item => ({
      ...item,
      name: AUTO_CORRECTIONS[item.name] ?? item.name,
    })),
  };
}
